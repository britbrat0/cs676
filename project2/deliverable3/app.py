...
from ai_helpers import generate_response_with_retry, generate_feedback_report, client as _client

# expose client at module level so tests can patch `app.client.responses.create`
client = _client


def generate_persona_responses(feature_inputs, personas, conversation_history, model_choice):
    """
    Test-importable function.
    Returns list of dicts: [{"persona": name, "response": text, "error": bool}, ...]
    """
    return generate_response_with_retry(feature_inputs, personas, conversation_history, model_choice)
...
def main():
    ...
    if api_key_input:
        ...
        try:
            ...
            # update module-level client to the real openai object
            global client
            client = openai
        except Exception:
            pass
    ...
    if ask_btn:
        if not st.session_state.api_key:
            ...
            with st.spinner("Generating persona responses..."):
                try:
                    results = generate_persona_responses(feature_inputs, selected_personas, st.session_state.conversation_history, model_choice)
                    # append results to conversation history in readable lines
                    for r in results:
                        if r.get("error"):
                            st.error(f"Failed to generate for {r.get('persona')}: {r.get('error_message','error')}")
                            st.session_state.conversation_history += f"\n{r.get('persona')}: - Response: [ERROR]\n"
                        else:
                            line = f"{r.get('persona')}: - Response: {r.get('response')}"
                            st.session_state.conversation_history += line + "\n"
                    st.rerun()
